  ...
  return (
    <div className="space-y-6 max-w-6xl mx-auto font-sans">
      <div className="flex items-baseline justify-between border-b border-[#DDD8D1] pb-4">
        <div>
          <h1 className="text-2xl font-bold text-[#1F1F1F]">Execution Queue</h1>
          <p className="text-xs text-[#6B6B6B] mt-1">
            Real-time pending and delayed email jobs managed by BullMQ Redis queues.
          </p>
        </div>
        <div className="flex items-center gap-2 text-xs text-[#1B7F4B] font-mono bg-[#FFFFFF] px-3 py-1.5 rounded-full border border-[#DDD8D1]">
          <RefreshCw className="w-3.5 h-3.5 animate-spin text-[#1B7F4B]" />
      ...
      <div className="bg-[#FFFFFF] border border-[#DDD8D1] rounded-lg overflow-hidden shadow-2xs">
        {isLoading ? (
          <div className="p-12 text-center text-[#6B6B6B] text-xs font-mono">
            ...
            Loading live queue from BullMQ...
          </div>
        ) : emails.length === 0 ? (
          <div className="p-12 text-center text-[#6B6B6B]">
            <Inbox className="w-8 h-8 mx-auto mb-2 opacity-40" />
            <p className="text-xs">No pending or delayed emails currently scheduled in queue.</p>
          </div>
        ) : (
          <table className="w-full text-left text-xs font-sans">
            <thead className="bg-[#FAF8F5] border-b border-[#DDD8D1] uppercase tracking-wider text-[#6B6B6B] text-[10px] font-bold">
              <tr>
                ...
                <th className="p-3.5">Scheduled Execution</th>
                <th className="p-3.5 text-right">Attempts</th>
              </tr>
            </thead>
            <tbody className="divide-y divide-[#DDD8D1] text-[#1F1F1F]">
              {emails.map((email) => (
                <tr key={email.id} className="hover:bg-[#FAF8F5] transition-colors">
                  ...
                  <td className="p-3.5">
                    <span className="inline-flex items-center gap-1 px-2.5 py-0.5 rounded-full text-[10px] uppercase font-bold bg-[#A46A00]/10 text-[#A46A00] border border-[#A46A00]/30">
                      <Clock className="w-3 h-3" />
                      {email.status}
                    </span>
                  </td>
                  <td className="p-3.5 text-[#6B6B6B] font-mono">{formatDate(email.scheduledFor)}</td>
                  <td className="p-3.5 text-right font-mono text-[#6B6B6B]">{email.attempts} / 3</td>
                </tr>
              ))}
            </tbody>
          </table>
