...
export function Header() {
  ...
  const handleGoogleLoginMock = () => {
    const newUser: UserProfile = {
      ...
      email: 'sarvagya@reachinbox.ai',
      avatarUrl: 'https://api.dicebear.com/7.x/avataaars/svg?seed=Sarvagya',
    };
  ...
  return (
    <header className="border-b border-[#DDD8D1] bg-[#FAF8F5] sticky top-0 z-40">
      ...
      <div className="max-w-[1280px] mx-auto px-8 h-[76px] flex items-center justify-between">
        ...
        {/* CENTER: Plain Text Navigation Links (No icons, no cards, no pills) */}
        <nav className="flex items-center space-x-8">
          {navItems.map((item) => {
            ...
            return (
              ...
              >
                <span>{item.name}</span>
                {/* Thin Burnt-Orange Baseline Active Indicator */}
                {isActive && (
